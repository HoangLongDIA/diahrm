...
class EmployeeController extends AccountBaseController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            ...
            $user->unionMember = $request->unionMember;
            $user->dangvien = $request->dangvien;
            $user->cuucb = $request->cuucb;
            $user->dqtv = $request->dqtv;
            $user->pccs = $request->pccs;
...
//            }
    ...
    public function show($id)
    {
        $this->viewPermission = user()->permission('view_employees');
            ...
            $this->employee = User::with(['employeeDetail.designation', 'employeeDetail.department', 'appreciations', 'appreciations.award', 'appreciations.award.awardIcon', 'employeeDetail.reportingTo', 'country',  'reportingTeam' => function ($query) {
            $query->join('users', 'users.id', '=', 'employee_details.user_id');
           // $query->join('user_insurance', 'user_insurance.id', '=', 'employee_details.insurance_id');
        ...
        //$test = $this->employee->user_trainings;
       //$test2 = $test;
        ...
        if (
            ...
            if ($tab == '') {  // Works for profile
                ...
                if (!is_null($this->employee->employeeDetail)) {
                    ...
                    $customFields1 = $customFields::where('name', 'nickname')->get();
                    if (!empty($customFields)) {
                        $this->fields = $customFields->fields;
                    }
                }
                $taskBoardColumn = TaskboardColumn::completeColumn();
                     //$hello = $this->fields::where('proxies', 1)->get();
        ...
        case 'training':
            ...
            $this->view = 'employees.ajax.training';
            break;
        case 'salary':
            $viewPermission = user()->permission('view_employee_salary');
            abort_403(!in_array($viewPermission, ['all']));
            $this->view = 'employees.ajax.salary';
            break;
        case 'appreciation':
        ...
        case 'leaves-quota':
            ...
            $this->insurances = DB::table('user_insurances')->where('user_id', $id)->orderByDesc('id')->paginate(2)->withQueryString();
            //dd($this->insurances);
